...
try:
    ...
    print '@me is following %d users' % (len(followings))
    print '\n'.join(["%s: %s" % (following.id, following.name) for following in followings])

    print '\nEnter user ID to unfollow (ENTER for \"@me\"): ',
    user_id = raw_input().strip()
    if (user_id == ''): user_id = '@me'
    buzz_client.unfollow(user_id)

    followings = buzz_client.following('@me').data
    print '@me is now following %d users' % (len(followings))
    print '\n'.join(["%s: %s" % (following.id, following.name) for following in followings])
