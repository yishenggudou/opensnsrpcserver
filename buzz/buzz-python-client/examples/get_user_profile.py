...
try:
    ...
    print '\n Now let\'s get an user profile. Please enter users\' ID (ENTER for \"@me\"): ',
    user_id = raw_input().strip()
    if (user_id == ''): user_id = '@me'

    print '\nUser %s' % user_id
    person = buzz_client.person(user_id).data
    print '\n'.join([
"""
Name:           %s
Photo:          %s
Uris:           %s
""" % (person.name, person.photo, person.uris)])

    posts = buzz_client.posts(user_id=user_id).data
    print 'Liked posts: '
    print '\n'.join([
"""
Post ID:        %s
...
Content         %s
URI:            %s
""" % (post.id, post.title, post.actor.name, post.content, post.link) for post in posts])
    print '\nAll done'
except:
